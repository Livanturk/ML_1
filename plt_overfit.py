...
from lab_utils_common import predict_logistic, plot_data, zscore_normalize_features
...
def plot_decision_boundary(ax, x0r, x1r, predict, w, b, scaler=False, mu=None, sigma=None, degree=None):
    ...
    Args:
        ...
        scaler : (boolean) scale data or not
    """
...
class OverfitExample:
    ...
    def __init__(self, regularize=False):
        ...
        self.bfitdata = Button(self.axfitdata, 'fit data', color='#1f77b4')
        ...
        self.typebut = ButtonManager(fig, [0.4, 0.07, 0.15, 0.15], ["Regression", "Categorical"],
                                     [False, True], self.toggle_type)

        self.fig.text(0.1, 0.02 + 0.21, "Degree", fontsize=12)
        self.degrbut = ButtonManager(fig, [0.1, 0.02, 0.15, 0.2], ['1', '2', '3', '4', '5', '6'],
                                     [True, False, False, False, False, False], self.update_equation)
        if self.regularize:
            self.fig.text(0.6, 0.02 + 0.21, r"lambda($\lambda$)", fontsize=12)
            self.lambut = ButtonManager(fig, [0.6, 0.02, 0.15, 0.2], ['0.0', '0.2', '0.4', '0.6', '0.8', '1'],
    ...
    def logistic_data(self, redraw=False):
        ...
        self.ax[0].text(0.5, 0.93, "Click on plot to add data. Hold [Shift] for blue(y=0) data.",
                        fontsize=12, ha='center', transform=self.ax[0].transAxes, color="#1f77b4")
    ...
    def linear_regression(self):
        ...
        x = np.linspace(*self.xlim, 30)  # plot line independent of data which gets disordered
...
if __name__ == "__main__":
    OverfitExample(regularize=True)
    plt.show()
