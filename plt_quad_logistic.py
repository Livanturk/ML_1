...
class PltQuadLogistic:
    ...
    def __init__(self, x_train, y_train, w_range, b_range):
        ...
        # initialize subplots
        self.dplot = DataPlot(ax[0], x_train, y_train, self.w, self.b)
        self.con_plot = ContourAndSurfacePlot(ax[1], ax[2], x_train, y_train, w_range, b_range, self.w, self.b)
        self.cplot = CostPlot(ax[3])
...
class DataPlot:
    ...
    def draw_logistic_lines(self, firsttime=False):
        ...
        self.aline = self.ax.plot(x_hat, y_hat, color=dlc["dlblue"], label="y = sigmoid(z)")
...
class ContourAndSurfacePlot:
    ...
    def __init__(self, axc, axs, x_train, y_train, w_range, b_range, w, b):
        ...
        self.path = Path(w, b, self.axc)  # initialize an empty path, avoids existence check
...
if __name__ == "__main__":
    x_train = np.array([0.5, 1.5, 2.0, 3.0, 3.5])
    y_train = np.array([0, 0, 1, 1, 1])
    w_range = (-1, 3)
    b_range = (-2, 2)

    plot = PltQuadLogistic(x_train, y_train, w_range, b_range)
    plt.show()
